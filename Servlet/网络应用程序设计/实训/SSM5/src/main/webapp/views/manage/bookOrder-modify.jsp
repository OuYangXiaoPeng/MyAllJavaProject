...
<div id="main" class="wrap">
    ...
    <div class="main">
        <h2>修改订单</h2>
        <div class="manage">
            <form action="modifyBookOrder.do">
                <input type="hidden" class="text" name="oid" value="${bookOrder.oid}"/>
                <table class="form">
                    <tr>
                        <td class="field">日期：</td>
                        <td><input type="date" class="text" name="date" value="${bookOrder.date}"/></td>
                    </tr>
                    <tr>
                        <td class="field">用户名：</td>
                        <td><input type="text" class="text" name="oname" value="${bookOrder.oname}"
                                   readonly="readonly"/></td>
                    </tr>
                    <tr>
                        <td class="field">地址：</td>
                        <td><input type="text" class="text" name="address" value="${bookOrder.address}"/></td>
                    </tr>
                    <tr>
                        <td class="field">状态：</td>
                        <td>
                            <input type="radio" name="status" value="已发货"
                                   <c:if test="${bookOrder.status=='已发货'}">checked="checked" </c:if>/>已发货
                            <input type="radio" name="status" value="未发货"
                                   <c:if test="${bookOrder.status=='未发货'}">checked="checked" </c:if>/>未发货
                        </td>
                    </tr>
                    <tr>
                        <td class="field">用户ID：</td>
                        <td><input type="text" class="text" name="uid" value="${bookOrder.uid}" readonly="readonly"/>
                        </td>
                    </tr>
